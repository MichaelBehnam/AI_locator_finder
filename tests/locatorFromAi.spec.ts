...
test('ask ai to generate a locator', async ({ page, aiLocator }) => {
    ...
    const usernameInput = await aiLocator("username input field");
    await usernameInput.fill("standard_user");
    
    // Get password input field using AI helper and fill it
    const passwordInput = await aiLocator("password input field");
    await passwordInput.fill("secret_sauce");
    
    // Get login button using AI helper and click it
    const loginButton = await aiLocator("login button");
    await loginButton.click();
    
    // Verify that we successfully logged in by checking the URL or page content
    await page.waitForURL("**/inventory.html");
    const inventoryContainer = await aiLocator("inventory container or list");
